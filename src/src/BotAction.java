...
public class BotAction {
	...
	public static void Click(int x, int y, int count) {
		...
		for(int c = count; c > 0; c--) {
			...
			bot.delay(AutoM8.pause);
			bot.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);
			bot.delay(AutoM8.pause);
		}
	}
	
	public static void ClickHold(int x, int y) {
		bot.mouseMove(x, y);
		bot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
		bot.delay(AutoM8.pause);
	}
	
	public static void ClickRelease(int x, int y) {
		bot.delay(AutoM8.pause);
		bot.mouseMove(x, y);
		bot.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);
		bot.delay(AutoM8.pause);
	...
	public static void Delay(int count, int value) {
		for (int c = count; c > 0; c--) {
			bot.delay(value);
		}
	}
	...
	public static void Tab(int count) {
		for(int c = count; c > 0; c--) {
			bot.keyPress(VK_TAB);
			bot.delay(AutoM8.pause);
			bot.keyRelease(VK_TAB);
			bot.delay(AutoM8.pause);
		}
	}
	
	public static void Enter(int count) {
		for(int c = count; c > 0; c--) {
	...
	// Type Stuff
	public static void Type(String strng) {
        int length = strng.length();
        for (int i = 0; i < length; i++) {
            char character = strng.charAt(i);
            Type(character);
//            if (!AutoM8.exec.execute) {
//            	//AutoM8GUI.mainDisplay.runPane.display.UpdateText("USER MANUAL OVERRIDE: HALT");
//            	break;
//            }
        }
    }
